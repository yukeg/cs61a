...
def subseqs(s):
    ...
    if not s:
        ...
        subseqs_without_first = subseqs(s[1:])
        return subseqs_without_first + insert_into_all(s[0], subseqs_without_first)
...
def inc_subseqs(s):
    ...
    """
    def subseq_helper(s, prev):   # prev is the previous s[0]
        if not s:
            return [[]]
        elif s[0] < prev:  
            return [[]]
        else:
            a = subseq_helper(s[1:], s[0])   # subsequences of s[1:]
