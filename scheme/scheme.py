...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        evaluated_operands = rest.map(lambda oper: scheme_eval(oper, env))
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    evaled = expressions.map(lambda oper: scheme_eval(oper, env))
    return convert(evaled)[-1]
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        formals, vals = convert(formals), convert(vals)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        validate_formals(f_formals)
        f_body = expressions.rest
        env.define(f_name, LambdaProcedure(f_formals, f_body, env))
        return f_name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    expressions = convert(expressions)
    evaled = None
    for expression in expressions:
        evaled = scheme_eval(expression, env)
        if is_false_primitive(evaled):
            return False
    return evaled if evaled else True
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            return val if val or val is nil else test
            # END PROBLEM 13
